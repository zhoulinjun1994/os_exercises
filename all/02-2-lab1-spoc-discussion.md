...
>  第一步：初始化IDT，设置时钟中断的中断描述符（Linux系统）
1. 调用init_ISA_IRQ()函数对中断向量32～256所对应的中断向量描述符进行初始化设置
2. 调用init_IRQ()函数设置中断向量32～256相对应的中断门
3. 调用init_IRQ()函数对PIT进行初始化编程
4. 调用sched_init()函数对计数器、时间中断的Bottom Half进行初始化
5. 调用time_init()函数对Linux内核的时钟中断机制进行初始化
第二步：
初始化8259中断控制器上的ICW1~ICW4控制关键字
第三步：
8253时钟EFLAG=0中断，时钟工作。
...
int main() 
{  
	func("DEMO", "This", "is", "a", "demo!" ,"333333", "\0");  
	return 0;
}  
```
...
> 先实现一个可显示字符串的bootloader，保证串口、并口、CGA的正常接入，执行工作有硬件设备初始化、分配RAM空间、设置堆栈、跳转到操作系统代码入口点
再实现一个可加载ELF格式的bootloader，注意对ELF格式的正确性要做出检查。执行工作有加载内核映像和设置内核映像执行参数。


如何能获取一个系统调用的调用次数信息？如何可以获取所有系统调用的调用次数信息？请简要说明可能的思路。(spoc)
- [x]  

> 在系统调用的底层函数中设置全局变量记录当前调用次数，每调用一次则加1
