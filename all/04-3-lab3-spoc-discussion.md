...
可以利用ucore中已经实现的循环链表结构pra\_list，访问控制可以由PTE_A来实现。但我认为，clock算法其实在性能上是不如LRU算法的，因为它是一个FIFO和LRU算法的折中。LRU算法在lab3基本不可能实现，因为没有实现相应的记录访问次数的变量，除非可以修改原有数据结构。
...
2. 写入只读段
去掉pmm.c中的get_pte中的地址PTE_W位
结果：陷入重启循环

3.找不到页目录项或页表项映射
在pmm.c的get_pte函数中返回NULL或在pmm.c的get_page函数中返回NULL
结果：陷入重启循环

4. 超过段界限的偏移量
修改entry.S第44行SEG_ASM的第三个参数为0
结果：操作系统崩溃

5. TSS不合法
在pmm.c的gdt_init函数中，注释第129行
结果：陷入重启循环
